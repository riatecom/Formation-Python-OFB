...
Dans les exemples précédents, nous avons déjà utilisé des méthodes :
...
Un attribut est une variable qui appartient à une classe. Elle est donc accessible depuis toutes les instances de la classe.
...
- Il existe de **nombreuses bibliothèques additionnelles** pour l'analyse de données (et il s'agit d'un écossystème très riche et vivant).
...
- [**NumPy**](https://numpy.org/) - Manipulation de tableaux de données typés
...
L'écosystème Python est très riche et il existe de nombreuses bibliothèques pour le traitement de données géospatiales.
...
Introduction à la programmation Python pour la biologie : https://python.sdv.univ-paris-diderot.fr/
...
:::{.column width="67%"}
**Objectif** : Créez un programme en Python qui permet de convertir la température de Celsius en Fahrenheit et vice versa.

**Instructions** :

- 1) Demandez à l'utilisateur de choisir le type de conversion qu'il souhaite effectuer : de Celsius à Fahrenheit ou de Fahrenheit à Celsius.

- 2) En fonction du choix de l'utilisateur, demandez-lui d'entrer la température à convertir.

- 3) Implémentez les formules de conversion appropriées :

    - De Celsius à Fahrenheit : $F = \frac{9}{5} \times C + 32$
    - De Fahrenheit à Celsius : $C = \frac{5}{9} \times (F - 32)$

- 4) Affichez le résultat de la conversion.

**Récupération d'une valeur entrée par l'utilisateur** :
...
- **Récupérez le notebook Jupyter [`wms-layers.ipynb`]() et ouvrez-le dans VSCodium.**
...
## Exercice 1 - Étendre les expressions QGIS avec Python

:::{.medium}

Dans cet exercice, vous créerez une fonction Python qui permet de calculer la distance entre deux points (un point fixe et un point mobile).

Cette fonction sera utilisée depuis la calculatrice de champ afin de créer un nouveau champ contenant la distance entre chaque point et un point fixe.

:::

## Exercice 2 - Manipuler des données vectorielles

:::{.medium}

Dans cet exercice, vous adapterez un script existant pour modifier un fichier vectoriel.
...
Dans cet exercice, vous créerez un algorithme de traitement qui permet ...
